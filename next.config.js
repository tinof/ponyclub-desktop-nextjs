...
const nextConfig = {
  ...
  // DEVELOPMENT NETWORK ACCESS: Configure allowed origins for cross-origin requests
  // This prevents warnings and future breaking changes when accessing dev server from network devices
  ...(process.env.NODE_ENV === "development" && {
    allowedDevOrigins: [
      // Allow access from local network IP (your current setup)
      "192.168.86.115:3000",
      // Allow common local network ranges for broader compatibility
      // Note: Next.js allowedDevOrigins only supports strings, not regex patterns
      // Using wildcard patterns for common private IP ranges
      "192.168.*.*:3000",
      "10.*.*.*:3000",
      "172.16.*.*:3000",
      "172.17.*.*:3000",
  ...
  async redirects() {
    return [
      // Domain canonicalization - redirect non-www to www
      {
        ...
        destination: "https://www.ponyclub.gr/:path*",
        permanent: true,
      },
      // 301 redirects from old WordPress URLs to new Next.js routes
      {
        source: "/el/activities/",
        destination: "/el/packages",
        permanent: true,
      },
      {
        source:
          "/el/%CF%81%CE%AC%CF%86%CF%84%CE%B9%CE%BD%CE%B3%CE%BA-%CE%BA%CE%B1%CE%B9-%CE%BA%CE%B1%CE%B3%CE%B9%CE%AC%CE%BA/",
        destination: "/el/rafting",
        permanent: true,
      },
      {
        source: "/for-schools",
        destination: "/en/for-schools",
        permanent: true,
      },
      { source: "/horse-riding/", destination: "/en/riding", permanent: true },
      {
