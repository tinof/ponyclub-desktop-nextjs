...
export const DEFAULT_IMAGE_SIZES = {
  ...
  thumbnail: '(max-width:480px)64px,(max-width:640px)80px,(max-width:768px)128px,192px',
...
export function optimizeImageProps(
  ...
  const { type = 'card', fetchPriority, enableBlurPlaceholder, ...rest } = props;
  ...
  if (fetchPriority) {
    (optimizedProps as any).fetchPriority = fetchPriority;
  }
