    ...
    const initializeBokun = () => {
      ...
      if (window.BokunWidgets) {
        if (process.env.NODE_ENV === 'development') {
          console.log('[Bokun Init] BokunWidgets found, checking for elements...')
        }
        ...
        if (widgets.length === 0) {
          if (process.env.NODE_ENV === 'development') {
            console.log('[Bokun Init] No Bokun elements found, skipping initialization')
          }
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log(`[Bokun Init] Found ${widgets.length} Bokun elements to initialize`)
        }
        ...
        try {
          ...
          if (typeof window.BokunWidgets.reinit === 'function') {
            ...
            if (process.env.NODE_ENV === 'development') {
              console.log('[Bokun Init] BokunWidgets.reinit() called successfully')
            }
            ...
            if (process.env.NODE_ENV === 'development') {
              console.log('[Bokun Init] BokunWidgets.scan() called successfully')
            }
          } else {
            // If no safe methods available, just log and let Bokun handle it naturally
            if (process.env.NODE_ENV === 'development') {
              console.log('[Bokun Init] No safe initialization method found, relying on natural initialization')
            }
          }
          ...
          if (process.env.NODE_ENV === 'development') {
            console.error('[Bokun Init] Error initializing Bokun widgets:', error)
          }
          return false
        }
      }
      ...
      if (initAttempts.current < maxAttempts) {
        if (process.env.NODE_ENV === 'development') {
          console.log(`[Bokun Init] BokunWidgets not ready yet, attempt ${initAttempts.current}/${maxAttempts}`)
        }
        setTimeout(initializeBokun, retryDelay)
      } else {
        if (process.env.NODE_ENV === 'development') {
          console.warn('[Bokun Init] Max attempts reached, Bokun widgets may not be available')
        }
      }
